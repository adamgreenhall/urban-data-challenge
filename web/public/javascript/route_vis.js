(function() {
  var colorOfDay, colorOfDayScale, colorOfText, colorOfTextScale, isNight, playbackTmax, updateTime;
  ...
  updateTime = function(timeDisplay, t) {
    ...
    return timeDisplay.weekday.text(d3.time.format.utc('%A')(curTime));
  };
  ...
  isNight = function(t) {
    ...
    console.log(hour, (0 <= hour && hour < 7) || hour >= 19);
    return (0 <= hour && hour < 7) || hour >= 19;
  };

  window.show_ts = function(error, data_daily, map) {
    var all_timers, begin_bus_trip, color_filler, force_layout, fraction_stopped_time, g, height, line, line_maker, margin, rScale, rVal, randomYpos, routePath, stopNameDisplay, stop_dists, stops, svg_route, tDepartureVal, tScale, tVal, timeDisplay, visStopRadius, vis_highlight_stop, vis_unhighlight_stop, width, xScale, xScaledValBus, xVal, yPos, yPosPassengers, yScale, yScaledValDoorsBus, yValBus, yValStop;
    ...
    begin_bus_trip = function(data_trip) {
      ...
      add_passenger_to_bus_stop = function(stop_number) {
      ...
      move_bus = function(stop_number) {
        ...
        arrival_fn = function() {
          ...
          if (stop_number < (data_stops.length - 1)) {
            ...
            if (passenger_circles[0].length === 0 && remaining[0].length > 0) {
      ...
      data_stops.forEach(function(stop, s) {
        return range(stop.count_boarding).forEach(function(el, i) {
          var tPrev;
